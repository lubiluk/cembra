...
class Actor(nn.Module):
    def __init__(self, num_actions, upper_bound):
        ...
        self._fc1 = nn.Linear(3000, 5000)
        self._norm1 = nn.LayerNorm(5000)
        self._fc2 = nn.Linear(5000, 1000)
        self._norm2 = nn.LayerNorm(1000)
        self._fc3 = nn.Linear(1000, num_actions)
...
class Critic(nn.Module):
    def __init__(self, num_actions):
        ...
        self._state_conv1 = nn.Conv2d(
            4, 16, kernel_size=3, stride=2, padding=1)
        self._state_conv2 = nn.Conv2d(
            16, 16, kernel_size=3, stride=2, padding=1)
        self._state_conv3 = nn.Conv2d(
            16, 10, kernel_size=3, stride=2, padding=1)
        self._state_fc1 = nn.Linear(3000, 5000)
        self._state_norm1 = nn.LayerNorm(5000)
        self._state_fc2 = nn.Linear(5000, 1000)
        self._state_norm2 = nn.LayerNorm(1000)

        self._action_fc1 = nn.Linear(num_actions, 32)
        self._action_norm1 = nn.LayerNorm(32)

        self._out_fc1 = nn.Linear(1032, 1024)
        self._out_norm1 = nn.LayerNorm(1024)
        self._out_fc2 = nn.Linear(1024, 1024)
        self._out_norm2 = nn.LayerNorm(1024)
        self._out_fc3 = nn.Linear(1024, 1)
...
class Learner3:
    def __init__(self):
        ...
        self.action_proxy = rospy.ServiceProxy(
            ACTION_SERVICE, cembra.srv.Action3)

        self._observation_processor = ObservationProcessor()
        self._frame_buffer = FrameBuffer(640, 480, 4)
        self._exp_replay = BigReplayBuffer(2000, (4, 480, 640), (8,))
    ...
    def _reset_env(self):
        ...
        image = self._observation_processor.process(response.image)
        image = self._frame_buffer.update(image)
    ...
    def _take_action(self, velocities):
        ...
        return (image, response.reward, response.is_done)
